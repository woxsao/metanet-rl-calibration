...
import numpy as np
from metanet_calibration.data_processing import plot_velocity
import os
...
if __name__ == "__main__":
    # command line args:
    import argparse
    parser = argparse.ArgumentParser(description="Run METANET calibration evaluation")
    parser.add_argument("--base_path", type=str, required=True, help="Path to input data. Should be a path to a data directory containing rho_hat.npy, q_hat.npy, lane_mapping.npy, etc.")
    parser.add_argument("--model_dir", type=str, required=True, help="Directory of the trained model to evaluate (should contain 'model.zip')")
    parser.add_argument("--figures_dir", type=str, default="./figures/", help="Directory to save output figures")
